...
Thumbnailer.prototype.execute = function(description, localPaths, onComplete) {
	...
	_.extend(this, {
		...
		colorSpace: (description.colorSpace || 'sRGB'),
...
Thumbnailer.prototype.matted = function() {
	var dimensionsString = this.width + 'X' + this.height,
	  qualityString = (this.quality ? '-quality ' + this.quality : ''),
		...
		thumbnailCommand = config.get('convertCommand') + ' "' + this.localPaths[0] + '[0]" ' + colorSpace + ' ' + autoOrient + ' -resize ' + dimensionsString + ' -size ' + dimensionsString + ' xc:' + this.background + ' +swap -gravity center' + qualityString + ' -composite ' + this.convertedPath;
...
Thumbnailer.prototype.bounded = function() {
	var dimensionsString = this.width + 'X' + this.height,
		...
		autoOrient = (this.autoOrient ? '-auto-orient' : ''),
		colorSpace = (this.colorSpace ? '-colorspace ' + this.colorSpace : ''),
		thumbnailCommand = config.get('convertCommand') + ' "' + this.localPaths[0] + '[0]" ' + colorSpace + ' ' + autoOrient + ' -thumbnail ' + dimensionsString + ' ' + qualityString + this.convertedPath;
...
Thumbnailer.prototype.fill = function() {
	var dimensionsString = this.width + 'X' + this.height,
		...
		autoOrient = (this.autoOrient ? '-auto-orient' : ''),
		colorSpace = (this.colorSpace ? '-colorspace ' + this.colorSpace : ''),
		thumbnailCommand = config.get('convertCommand') + ' "' + this.localPaths[0] + '[0]" ' + colorSpace + ' ' + autoOrient + (this.background ? ' -background '+this.background : '') + ' -resize ' + dimensionsString + '^ -gravity center -extent ' + dimensionsString + ' ' + qualityString + ' ' + this.convertedPath;
...
Thumbnailer.prototype.strict = function() {
	var dimensionsString = this.width + 'X' + this.height,
		...
		autoOrient = (this.autoOrient ? '-auto-orient' : ''),
		colorSpace = (this.colorSpace ? '-colorspace ' + this.colorSpace : ''),
		thumbnailCommand = config.get('convertCommand') + ' "' + this.localPaths[0] + '[0]" ' + colorSpace + ' ' + autoOrient + ' -resize ' + dimensionsString + '! ' + qualityString + ' ' + this.convertedPath;
